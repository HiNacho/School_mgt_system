...
export default function DashboardHome() {
  ...
  // Load dismissed alert IDs on mount
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const stored = localStorage.getItem('report_dismissed_fees_alerts');
        if (stored) {
  ...
  // Dismiss fees alert locally
  const handleDismissFeesAlert = (id: string) => {
    const updated = [...dismissedAlertIds, id];
    setDismissedAlertIds(updated);
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('report_dismissed_fees_alerts', JSON.stringify(updated));
      } catch (e) {
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                ...
                <span className="text-[9px] font-extrabold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded uppercase tracking-wider">APPROVAL & PARENT RELEASE CONTROL</span>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                    {(() => {
                      ...
                      return classStatuses.map((item) => {
                        ...
                        let statusText = 'Draft / Not Submitted';
                        if (item.status === 'NOT_SET' || item.status === 'DRAFT') {
                          statusColor = 'bg-blue-50 text-blue-700 border-blue-200';
                          statusText = 'Draft / Not Submitted';
                        } else if (item.status === 'AWAITING_APPROVAL') {
                          statusColor = 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse font-black';
                          statusText = 'Pending Admin Approval';
                        } else if (item.status === 'APPROVED') {
                          statusColor = 'bg-emerald-50 text-emerald-700 border-emerald-200 font-black';
                        ...
                        const statusKey = `${item.classId}_${item.armId}`;
                        const isUpdating = updatingStatusMap[statusKey];
                        ...
                        return (
                          <tr key={`${item.classId}-${item.armId}`} className="hover:bg-slate-50/40 transition-colors">
                            ...
                            <td className="p-3.5 text-slate-600 font-medium">
                              {item.classTeacherName || 'Unassigned'}
                            </td>
                            ...
                            <td className="p-3.5">
                              <div className="flex flex-wrap items-center gap-2">
                                {isAdmin && (item.status === 'AWAITING_APPROVAL' || item.status === 'DRAFT' || item.status === 'REJECTED' || item.status === 'NOT_SET') && (
                                  <button
                                    type="button"
                                    disabled={isUpdating}
                                    onClick={() => handleAdminUpdateStatus(item.classId, item.armId, 'APPROVED')}
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-50"
                                    title="Approve and officially release report cards to parents"
                                  >
                                    <CheckCircle className="w-3.5 h-3.5" /> Approve & Send to Parents
                                  </button>
                                )}

                                {isAdmin && item.status === 'AWAITING_APPROVAL' && (
                                  <button
                                    type="button"
                                    disabled={isUpdating}
                                    onClick={() => {
                                      const reason = prompt('Please enter correction feedback for the Class Teacher:');
                                      if (reason !== null) {
                                        handleAdminUpdateStatus(item.classId, item.armId, 'REJECTED', reason);
                                      }
                                    }}
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 disabled:opacity-50"
                                  >
                                ...
                                {isAdmin && item.status === 'APPROVED' && (
                                  <button
                                    type="button"
                                    disabled={isUpdating}
                                    onClick={() => handleAdminUpdateStatus(item.classId, item.armId, 'DRAFT')}
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-250 disabled:opacity-50"
                                  >
                                    Revoke Release
                                  </button>
                                )}
                                ...
                                <a
                                  ...
                                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all border bg-white hover:bg-slate-50 border-slate-200 text-slate-650 hover:text-slate-805"
                                >
                                  Inspect Compiler <Eye className="w-3 h-3 text-slate-400" />
                                </a>
                              </div>
                            </td>
                          </tr>
      ...
      {isParent && (
        <div className="space-y-6">
          ...
          {selectedChildId ? (() => {
            ...
            const latestFeesAnnouncement = [...announcements]
              .filter((a: any) => 
                a.title.toLowerCase().includes('fees') || 
                a.title.toLowerCase().includes('payment') || 
                a.content.toLowerCase().includes('fees') || 
                a.content.toLowerCase().includes('payment')
              )
              .sort((a, b) => new Date(b.createdAt || b.date).getTime() - new Date(a.createdAt || a.date).getTime())[0];

            const alertId = latestFeesAnnouncement ? latestFeesAnnouncement.id : `default-fees-${kid.id}`;
            const isDismissed = dismissedAlertIds.includes(alertId);

            return (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                
                {/* Red Outstanding Fees Alert Banner */}
                {(!isDismissed && (!kid.feesPaid || activeStudentDetail?.invoices?.some((inv: any) => inv.status === 'OUTSTANDING' || inv.status === 'PARTIALLY_PAID'))) && (
                  <div className="lg:col-span-12 bg-rose-50 border border-rose-150 p-5 rounded-3xl flex justify-between items-start gap-3 text-rose-800 animate-in fade-in duration-300">
                    <div className="flex gap-3">
                      <AlertCircle className="w-5 h-5 text-rose-600 flex-shrink-0 mt-0.5" />
                      <div className="flex-1 space-y-2">
                        <h4 className="text-xs font-black uppercase tracking-wider text-rose-800">
                          {latestFeesAnnouncement ? latestFeesAnnouncement.title : "Outstanding School Fees Alert"}
                        </h4>
                        <p className="text-[11px] text-rose-700 font-semibold leading-relaxed">
                          {latestFeesAnnouncement 
                            ? latestFeesAnnouncement.content 
                            : `Our records show that your child ${kid.firstName} ${kid.lastName} has outstanding fees for this term. Please review the invoices below and settle the balance via card payment.`
                          }
                        </p>
                        <p className="text-[11px] text-rose-700 font-semibold leading-relaxed">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDismissFeesAlert(alertId)}
                      className="p-1 hover:bg-rose-100 rounded-lg text-rose-450 hover:text-rose-700 transition-all cursor-pointer flex-shrink-0"
                      title="Dismiss Alert"
                    >
                      <X className="w-4 h-4" />
                    </button>
